...
func TestTemplateEngine_TruncateEdgeCases(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "Limit equals 4 (first truncation marker case)",
			title:    "Test Movie",
			maxLen:   4,
			expected: "T...",
		},
...
func TestTemplateEngine_TruncateTitleBytes(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			comment:  "Reserves 3 bytes for truncation marker, breaks at word boundary",
		},
		{
			...
			comment:  "No spaces, truncates mid-word with marker within budget",
		},
		{
			name:     "ASCII title - very small limit with marker",
			title:    "Test Movie Title",
			maxBytes: 5,
			expected: "Te...",
			comment:  "Budget of 4 bytes for content + 3 for marker = 7 total",
		},
		{
			name:     "ASCII title - small limit still allows marker",
			title:    "Test Movie Title",
			maxBytes: 2,
			expected: "Te",
			comment:  "Only 2 bytes, no room for marker with legacy reserve",
		},
		{
			...
			maxBytes: 20,
			expected: "これは日本...",
			comment:  "Budget 17 bytes = 4 CJK chars (12 bytes) + marker (3 bytes) = 15 bytes, TrimRight space + marker",
		},
		{
		...
		},
		{
			...
			maxBytes: 11,
			expected: "これ...",
			comment:  "Budget 8 bytes = 2 CJK chars (6 bytes) + marker (3 bytes) = 9 bytes, TrimRight space + marker",
		},
		{
			...
			comment:  "Budget 17 bytes: 'Movie Title ' (12) + 1 CJK (3) + marker (3) = 18 bytes, word boundary trim",
		},
		{
		...
		{
			name:     "ASCII with marker edge case",
			title:    "Test Movie Title",
			maxBytes: 6,
			expected: "Tes...",
			comment:  "Budget 3 bytes for content + 3 for marker = 6 total",
		},
		{
			name:     "Korean characters",
			title:    "한국어 제목입니다",
			maxBytes: 15,
			expected: "한국어...",
			comment:  "Budget 12 bytes = 3 Korean chars (9 bytes) + marker (3 bytes) = 12 bytes",
		},
		{
			name:     "Chinese characters",
			title:    "这是一个很长的中文标题",
			maxBytes: 24,
			expected: "这是一个很长的...",
			comment:  "Budget 21 bytes = 7 CJK chars (21 bytes) + marker (3) = 24 total",
		},
		{
			...
			maxBytes: 12,
			expected: "The...",
			comment:  "Budget 9 bytes breaks at last space and appends marker",
		},
		{
			name:     "Title shorter than limit",
			title:    "Short",
		...
		},
		{
			...
			maxBytes: 3,
			expected: "Tes",
			comment:  "No room for marker, just 3 chars",
		},
	}
