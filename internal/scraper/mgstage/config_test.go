...
func TestFlattenFunc_WithValidConfig(t *testing.T) {
	fn := scraperutil.GetFlattenFunc("mgstage")
	require.NotNil(t, fn)

	result := fn(&mockScraperConfig{enabled: true, requestDelay: 500})
	require.NotNil(t, result)

	settings, ok := result.(*config.ScraperSettings)
	require.True(t, ok)
	assert.True(t, settings.Enabled)
	assert.Equal(t, 500, settings.RateLimit)
}
...
func TestValidateConfig(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantErr: true,
			errMsg:  "mgstage: rate_limit must be non-negative, got -1",
		},
		{
		...
		},
		{
			...
			wantErr: true,
			errMsg:  "mgstage: retry_count must be non-negative, got -1",
		},
		{
		...
		},
		{
			...
			wantErr: true,
			errMsg:  "mgstage: timeout must be non-negative, got -1",
		},
		{
...
func TestGetterMethods(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			config:         &MGStageConfig{BaseScraperConfig: config.BaseScraperConfig{Enabled: true}},
			getter:         "IsEnabled",
			expectedResult: true,
		},
		{
			name:           "IsEnabled returns false when disabled",
			config:         &MGStageConfig{BaseScraperConfig: config.BaseScraperConfig{Enabled: false}},
			getter:         "IsEnabled",
			expectedResult: false,
		},
		{
			name:           "GetUserAgent returns custom user agent",
			config:         &MGStageConfig{BaseScraperConfig: config.BaseScraperConfig{UserAgent: "custom-agent/1.0"}},
			getter:         "GetUserAgent",
			expectedResult: "custom-agent/1.0",
		},
		{
			name:           "GetUserAgent returns empty string when not set",
			config:         &MGStageConfig{BaseScraperConfig: config.BaseScraperConfig{UserAgent: ""}},
			getter:         "GetUserAgent",
			expectedResult: "",
		},
		{
			name:           "GetRequestDelay returns configured delay",
			config:         &MGStageConfig{BaseScraperConfig: config.BaseScraperConfig{RequestDelay: 1000}},
			getter:         "GetRequestDelay",
			expectedResult: 1000,
		},
		{
			name:           "GetRequestDelay returns 0 when not set",
			config:         &MGStageConfig{BaseScraperConfig: config.BaseScraperConfig{RequestDelay: 0}},
			getter:         "GetRequestDelay",
			expectedResult: 0,
		},
		{
			name:           "GetMaxRetries returns configured retry count",
			config:         &MGStageConfig{BaseScraperConfig: config.BaseScraperConfig{MaxRetries: 3}},
			getter:         "GetMaxRetries",
			expectedResult: 3,
		},
		{
			name:           "GetMaxRetries returns 0 when not set",
			config:         &MGStageConfig{BaseScraperConfig: config.BaseScraperConfig{MaxRetries: 0}},
			getter:         "GetMaxRetries",
			expectedResult: 0,
		},
		{
			name:           "GetProxy returns proxy config when set",
			config:         &MGStageConfig{BaseScraperConfig: config.BaseScraperConfig{Proxy: proxyConfig}},
			getter:         "GetProxy",
			expectedResult: proxyConfig,
		},
		{
			name:           "GetProxy returns nil when not set",
			config:         &MGStageConfig{BaseScraperConfig: config.BaseScraperConfig{Proxy: nil}},
			getter:         "GetProxy",
			expectedResult: nil,
		},
		{
			name:           "GetDownloadProxy returns download proxy config when set",
			config:         &MGStageConfig{BaseScraperConfig: config.BaseScraperConfig{DownloadProxy: downloadProxyConfig}},
			getter:         "GetDownloadProxy",
			expectedResult: downloadProxyConfig,
		},
		{
			name:           "GetDownloadProxy returns nil when not set",
			config:         &MGStageConfig{BaseScraperConfig: config.BaseScraperConfig{DownloadProxy: nil}},
			getter:         "GetDownloadProxy",
			expectedResult: nil,
		},
	}
