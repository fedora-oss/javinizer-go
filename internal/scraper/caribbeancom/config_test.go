...
func TestGetterMethods(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			config:         &CaribbeancomConfig{BaseScraperConfig: config.BaseScraperConfig{Enabled: true}},
			getter:         "IsEnabled",
			expectedResult: true,
		},
		{
			name:           "IsEnabled returns false when disabled",
			config:         &CaribbeancomConfig{BaseScraperConfig: config.BaseScraperConfig{Enabled: false}},
			getter:         "IsEnabled",
			expectedResult: false,
		},
		{
			name:           "GetUserAgent returns custom user agent",
			config:         &CaribbeancomConfig{BaseScraperConfig: config.BaseScraperConfig{UserAgent: "custom-agent/1.0"}},
			getter:         "GetUserAgent",
			expectedResult: "custom-agent/1.0",
		},
		{
			name:           "GetUserAgent returns empty string when not set",
			config:         &CaribbeancomConfig{BaseScraperConfig: config.BaseScraperConfig{UserAgent: ""}},
			getter:         "GetUserAgent",
			expectedResult: "",
		},
		{
			name:           "GetRequestDelay returns configured delay",
			config:         &CaribbeancomConfig{BaseScraperConfig: config.BaseScraperConfig{RequestDelay: 1000}},
			getter:         "GetRequestDelay",
			expectedResult: 1000,
		},
		{
			name:           "GetRequestDelay returns 0 when not set",
			config:         &CaribbeancomConfig{BaseScraperConfig: config.BaseScraperConfig{RequestDelay: 0}},
			getter:         "GetRequestDelay",
			expectedResult: 0,
		},
		{
			name:           "GetMaxRetries returns configured retry count",
			config:         &CaribbeancomConfig{BaseScraperConfig: config.BaseScraperConfig{MaxRetries: 3}},
			getter:         "GetMaxRetries",
			expectedResult: 3,
		},
		{
			name:           "GetMaxRetries returns 0 when not set",
			config:         &CaribbeancomConfig{BaseScraperConfig: config.BaseScraperConfig{MaxRetries: 0}},
			getter:         "GetMaxRetries",
			expectedResult: 0,
		},
		{
			name:           "GetProxy returns proxy config when set",
			config:         &CaribbeancomConfig{BaseScraperConfig: config.BaseScraperConfig{Proxy: proxyConfig}},
			getter:         "GetProxy",
			expectedResult: proxyConfig,
		},
		{
			name:           "GetProxy returns nil when not set",
			config:         &CaribbeancomConfig{BaseScraperConfig: config.BaseScraperConfig{Proxy: nil}},
			getter:         "GetProxy",
			expectedResult: nil,
		},
		{
			name:           "GetDownloadProxy returns download proxy config when set",
			config:         &CaribbeancomConfig{BaseScraperConfig: config.BaseScraperConfig{DownloadProxy: downloadProxyConfig}},
			getter:         "GetDownloadProxy",
			expectedResult: downloadProxyConfig,
		},
		{
			name:           "GetDownloadProxy returns nil when not set",
			config:         &CaribbeancomConfig{BaseScraperConfig: config.BaseScraperConfig{DownloadProxy: nil}},
			getter:         "GetDownloadProxy",
			expectedResult: nil,
		},
	}
